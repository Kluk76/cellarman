...
- **Journal discipline:** dated change-log entries go to the journal topic file (newest-first) — the index keeps ONLY the current head pointer + still-armed warnings. Dated arc narration goes to the arc topic file under `## Build log` — the index arc line stays a ONE-LINER: name · status · compressed rails · triggers · pointer. Rails are COMPRESSED, never dropped; narration moves, never dies. A journal bucket ROTATES at ~85% of the topic ceiling — it never restructures.
- **The router in your index is a shortlist, not the corpus.** When no index line names a file for the task's domain, run `claude-brain/pm-kit/catalog.sh --grep '<terms>'` BEFORE concluding a subject has no memory — it regenerates the card catalog of every topic file and greps it. Every topic file you create gets a `> Trigger …` blockquote directly under its title (the catalog harvests it); `catalog.sh --audit` lists files lacking one.
