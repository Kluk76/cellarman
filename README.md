...
without bound because it's only loaded on demand. Memory writes itself back:
...
5. Gitignore the load log (telemetry is per-machine by design).
...
- **Verbatim relocation, wholesale snapshot first.** Every compaction starts
  ...
  is never paraphrased at move time.
